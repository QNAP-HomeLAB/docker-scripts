  ...
  fnc_folder_create(){
    ...
    for stack in "${folder_list[@]}"; do
      if [ ! -d "${docker_appdata}/${stack}/" ];
        ...
        else echo -e "  ${ylw:?}ALREADY EXISTS ${def:?}> ${CYN:?}${docker_appdata}${cyn:?}/${stack}${def:?}"; exist=1; # ((++exist));
      fi;
      if [ ! -f "${docker_appdata}/${stack}/${stack}.log" ];
        then "${var_sudo}"install -m 660 /dev/null "${docker_appdata}/${stack}/${stack}-logs.yml";
        # then touch "${docker_compose}/${stack}/${stack}.log" && "${var_sudo}"chmod 660 "${docker_compose}/${stack}/${stack}.log";
        else echo -e "  ${ylw:?}ALREADY EXISTS ${def:?}> ${CYN:?}${docker_appdata}/${cyn:?}${stack}/${stack}-logs.yml${def:?}";
      fi;
      ...
      #   else echo -e "  ${ylw:?}ALREADY EXISTS${def:?} > ${CYN:?}${docker_appdata}/${cyn:?}${stack}/certs/acme.json${def:?}";
      # fi;
      if [ ! -d "${docker_compose}/${stack}/" ];
        then "${var_sudo}"install -o "${var_uid}" -g "${var_gid}" -m 664 -d "${docker_compose}/${stack}";
        else echo -e "  ${ylw:?}ALREADY EXISTS ${def:?}> ${CYN:?}${docker_compose}${cyn:?}/${stack}${def:?}"; exist=1; # ((++exist));
      fi;
      if [ ! -f "${docker_compose}/${stack}/compose.yml" ];
        then "${var_sudo}"install -m 664 /dev/null "${docker_compose}/${stack}/compose.yml";
        # then touch "${docker_compose}/${stack}/compose.yml" && "${var_sudo}"chmod 664 "${docker_compose}/${stack}/compose.yml";
          { printf "# '%s' docker config file created for a homelab environment - https://github.com/qnap-homelab\n\n" "${stack}";
          } >> "${docker_compose}/${stack}/compose.yml";
        else echo -e "  ${ylw:?}ALREADY EXISTS ${def:?}> ${CYN:?}${docker_compose}/${cyn:?}${stack}/compose.yml";
      fi;
