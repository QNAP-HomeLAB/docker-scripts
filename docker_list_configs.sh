...
# function definitions
  fnc_help(){
    ...
    echo -e " -     ${cyn}-c │ --compose ${DEF}│ Displays stacks with config files in the ${YLW}..${docker_compose}/${def} filepath."
    echo -e " -     ${cyn}-w │ --swarm   ${DEF}│ Displays stacks with config files in the ${YLW}..${docker_swarm}/${def} filepath."
    echo -e " -"
  ...
  fnc_type_compose(){ configs_path="${docker_compose}"; conftype="-compose"; }
  fnc_type_swarm(){ configs_path="${docker_swarm}"; unset conftype IFS; }
...
# determine configuration type to query
  case "$1" in
    ...
    "-c"|"--compose")
      ...
      fnc_type_compose
      fnc_script_intro_compose
      ;;
    "-s"|"-w"|"--swarm")
      source /opt/docker/swarm/.stackslist-swarm.conf
      fnc_type_swarm
      fnc_script_intro_swarm
      ;;
    *)
      fnc_invalid_syntax
      ;;
