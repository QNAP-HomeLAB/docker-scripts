...
# function definitions
  ...
  fnc_network_create(){
    ...
    case "${scope}" in
      ("local")
        if [ ! "$(fnc_network_check_by_name "docker_gwbridge")" = "" ]; then docker network rm "docker_gwbridge"; fi;
        ;;
      ("swarm")
        if [ ! "$(fnc_network_check_by_name "ingress")" = "" ]; then docker network rm ingress; fi;
