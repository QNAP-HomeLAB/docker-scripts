...
# function definitions
  fnc_help(){
    ...
    # echo -e " -       ${cyn}-d │ --delete ${DEF}│ ${red}Deletes${def} all sub-folders and files in ${YLW}${swarm_appdata}/${cyn}swarm_stack${DEF} & ${YLW}${swarm_configs}/${cyn}swarm_stack${DEF}"
    # echo -e " -       ${cyn}-r │ --reset  ${DEF}│ ${red}Deletes${def} all files contained in ${YLW}${swarm_appdata}/${cyn}swarm_stack${DEF}"
  ...
  fnc_folder_ownership_update(){ chown -R ${var_usr}:${var_grp} ${swarm_folder}; chmod 600 -c {$swarm_appdata,$swarm_configs}/{$1,$2,$3,$4,$5,$6,$7,$8,$9}; }
  ...
  fnc_swarm_folders_create(){ mkdir -p {$swarm_appdata,$swarm_configs}/{$1,$2,$3,$4,$5,$6,$7,$8,$9}; }
  fnc_swarm_delete_folders(){ rmdir {$swarm_appdata,$swarm_configs}/{$2,$3,$4,$5,$6,$7,$8,$9}; }
  fnc_swarm_delete_appdata(){ 
    rm -i ${swarm_appdata}/{$2,$3,$4,$5,$6,$7,$8,$9}; 
    while read -r -p " - Also delete the ${YLW}${swarm_appdata}/${cyn}swarm_stack${DEF} folder? [(Y)es/(N)o] " input; do
      case "${input}" in 
  ...
  fnc_swarm_delete_configs(){ 
    rm -i {$swarm_appdata,$swarm_configs}/{$2,$3,$4,$5,$6,$7,$8,$9}; 
    while read -r -p " - Also delete the ${YLW}${swarm_configs}/${cyn}swarm_stack${DEF} folder? [(Y)es/(N)o] " input; do
      case "${input}" in 
        ([yY]|[yY][eE][sS]) fnc_swarm_delete_folders $2 $3 $4 $5 $6 $7 $8 $9 ;;
        ([nN]|[nN][oO]) break ;;
        (*) fnc_invalid_input ;;
  ...
  case "${1}" in 
    ...
    (-*) # validate and perform option
      case "${1}" in
        ...
        ("-d"|"-del"|"--delete") fnc_swarm_delete_configs $2 $3 $4 $5 $6 $7 $8 $9 ;;
        ("-r"|"-res"|"--reset") fnc_swarm_delete_appdata $2 $3 $4 $5 $6 $7 $8 $9 ;;
        (*) fnc_invalid_syntax ;;
      esac
    ;;
    (*)
